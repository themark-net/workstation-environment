...
jq -nc --argjson schema 1 --argjson ts "$ts" --arg hostname "$hostname_f" --arg device_id "$device_id" --argjson tpm_present "$tpm_present" --argjson attested "$attested" --argjson ticket_expires_at "${ticket_exp:-0}" --argjson lab false '{schema:$schema,ts:$ts,hostname:$hostname,device_id:$device_id,tpm_present:$tpm_present,attested:$attested,ticket_expires_at:$ticket_expires_at,lab:$lab}' >"${STATUS_JSON}.tmp"
...
echo "ltz-trust-agent: attested=$attested status=$STATUS_JSON"
